      ...
      <Box sx={{ flexGrow: 1 }}>
                  ...
                  <List>
                    {getFilteredCommunications().length > 0 ? (
                      getFilteredCommunications().map((communication) => (
                        <ListItem
                          key={`${communication.studentId}-${communication.date}-${communication.subject}`}
                            ...
                            <Box>
                              {communication.sentStatus === "Draft" && (
                                <Tooltip title="Send">
                                  <IconButton
                                    color="success"
                                    onClick={() => handleSendDraft(communication)}
                                    size="small"
                                  >
                                    <SendIcon />
                                  </IconButton>
                          ...
                          <ListItemText
                            primary={
                              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                <Chip
                                  icon={getStatusIcon(communication.sentStatus)}
                                  label={communication.sentStatus}
                                  color={getStatusColor(communication.sentStatus)}
                                  size="small"
                                />
        ...
        >
          ...
          <DialogActions>
            ...
            <Button
              ...
              startIcon={<SaveIcon />}
              disabled={
                !emailForm.studentId || !emailForm.subject || !emailForm.content
              }
            >
              Save Draft
            </Button>
        ...
        >
                  ...
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                    {emailTemplates.map((template, index) => (
                      <Chip
                        key={index}
                        label={template.name}
                        onClick={() => handleApplyBulkTemplate(template)}
                        color="primary"
                        variant="outlined"
                      />
                    ))}
          ...
          <DialogActions>
            ...
            <Button
              onClick={() => handleSaveBulkEmail(false)}
              variant="outlined"
              color="primary"
              startIcon={<SaveIcon />}
              disabled={
                ...
                !bulkEmailForm.content
              }
            >
              Save Drafts
            </Button>
            <Button
              onClick={() => handleSaveBulkEmail(true)}
              variant="contained"
              color="primary"
              startIcon={<SendIcon />}
              disabled={
            ...
            {selectedCommunication && (
              <Box sx={{ mt: 2 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  To: {getStudentName(selectedCommunication.studentId)}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Date: {formatDate(selectedCommunication.date)}
                </Typography>
              </Box>
            )}
          </DialogContent>

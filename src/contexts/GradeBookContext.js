...
export const GradeBookProvider = ({ children }) => {
  ...
  const loadGradeBooks = async () => {
    try {
      ...
      // Only load gradebooks from Firestore
      const gradebooksQuery = query(
        collection(db, "gradebooks"),
        where("teacherId", "==", user.uid)
      );
      const snapshot = await getDocs(gradebooksQuery);
      const gradebooksData = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
  ...
  const updateGradeBook = async (gradeBookId, updates) => {
    try {
      ...
      setGradeBooks((prev) =>
        prev.map((gb) =>
          gb.id === gradeBookId ? { ...gb, ...updateData } : gb
        )
      );
      ...
      if (gradeBookSnap.exists()) {
        ...
        setGradeBooks((prev) => {
          if (prev.find((gb) => gb.id === fetchedGradeBook.id)) {
            return prev.map((gb) =>
              gb.id === fetchedGradeBook.id ? fetchedGradeBook : gb
            );
          }
  ...
  const duplicateGradeBook = async (gradeBookId) => {
    try {
      ...
      const duplicatedData = {
        ...
        categories:
          originalGradeBook.categories?.map((cat) => ({
            ...cat,
  ...
  const addCategoryToGradeBook = async (gradeBookId, categoryData) => {
    try {
      ...
      const newTotalWeight =
        gradeBook.categories.reduce((sum, cat) => sum + cat.weight, 0) +
        categoryData.weight;
      ...
      try {
        ...
        console.warn(
          "Failed to save to Firestore, using local storage only:",
          firestoreError
        );
      }
  ...
  const updateCategoryInGradeBook = async (
    gradeBookId,
    categoryId,
    updates
  ) => {
    try {
      ...
      const categoryIndex = gradeBook.categories.findIndex(
  ...
  const removeCategoryFromGradeBook = async (gradeBookId, categoryId) => {
    try {
      ...
      const hasAssignments = assignments.some(
        (assignment) =>
          assignment.subject === gradeBook.subject &&
          assignment.category === categoryToRemove.name
      );

      if (hasAssignments) {
        throw new Error(
          "Cannot remove category that has assignments. Please reassign or delete assignments first."
        );
      }

      const updatedCategories = gradeBook.categories.filter(
        (cat) => cat.id !== categoryId
      );
      const updatedCategoryWeights = { ...gradeBook.categoryWeights };
  ...
  const reorderCategoriesInGradeBook = async (gradeBookId, categoryIds) => {
    try {
      ...
      const isValidOrder = categoryIds.every((id) =>
      ...
      const reorderedCategories = categoryIds
        .map((id) => gradeBook.categories.find((cat) => cat.id === id))
        .filter(Boolean);
  ...
  const value = {
    ...
    // createTermGradeBook, // removed
    // cloneGradeBookForNewTerm, // removed
